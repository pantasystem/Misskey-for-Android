...
class Description(){
    ...
    fun getOriginNotes(note: Note, callBack: (timeline: List<NoteViewData>?) -> Unit) = GlobalScope.launch{
        try{
            ...
            replyList.add(NoteViewData(note = note, isReply = false, isOriginReply = false, type = AbsTimeline.NoteType.NOTE, reactionCountPairList = ReactionCountPair.createList(note.reactionCounts!!)))
            while(reply != null){
                ...
                replyList.add(NoteViewData(note = reply, isReply = false, isOriginReply = false, type = AbsTimeline.NoteType.NOTE, reactionCountPairList = reactionPair))
                reply = reply.reply
            }
